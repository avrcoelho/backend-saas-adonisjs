...
class TeamController {
   ...
   */
  async index ({ auth }) {
    // retorna todos os times do usário
  ...
  async store ({ request, auth }) {
    ...
    // ja faz o relacionamento de usuario com time
    const team = await auth.user.teams().create({
      ...data,
      user_id: auth.user.id
    })

    const teamJoin = await auth.user.teamJoins('team_id', team.id).first()

    const admin = await Role.find.findBy('slug', 'administrator')
